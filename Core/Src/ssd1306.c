...
extern I2C_HandleTypeDef hi2c1; // Make sure this matches your CubeMX setup (usually hi2c1)
...
char SSD1306_Puts(char* str, FontDef_t* Font, SSD1306_COLOR_t color) {
	while (*str) {
		if (SSD1306_Putc(*str, Font, color) != *str) return *str;
		str++;
	}
	return *str;
}
...
void SSD1306_DrawFilledRectangle(uint16_t x, uint16_t y, uint16_t w, uint16_t h, SSD1306_COLOR_t c) {
	...
	if ((x + w) >= SSD1306_WIDTH) w = SSD1306_WIDTH - x;
	if ((y + h) >= SSD1306_HEIGHT) h = SSD1306_HEIGHT - y;
	for (i = 0; i <= h; i++) SSD1306_DrawLine(x, y + i, x + w, y + i, c);
}
